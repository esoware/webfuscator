...
`vitest.config.ts` maps the `src/*` alias and raises the worker stack with `--stack-size`. The deep-nesting tests need the larger stack because `@babel/parser` exhausts Node's default while parsing them.

`build` removes `dist` through Node's filesystem API so the script works in both Windows and POSIX shells.

## Pipeline and transforms

The phase tables in `src/obfuscator.ts` define which passes run and in what order. Their comments explain every ordering constraint. When you add, move, or reorder a pass, update the tables and those comments. Do not record pipeline order anywhere else.
...
- Track changes through `traverseForChanges` and `ChangeState` in `src/utils/change-tracking.ts`. A pass with its own traversal loop keeps state in an object that extends `ChangeState`.
- Give every new pass in `src/transforms/` or `src/preparation/` a mirror test at the same path under `tests/`. If the pass is configurable, add it to `TransformName` in `src/options.ts`.
- Most helpers in `src/utils/` and `src/analysis/` get coverage from their transforms. The rewrite safety predicates in `src/analysis/purity.ts`, `src/analysis/constant.ts`, and `src/analysis/document-order.ts` need direct mirror tests. So do the value builders in `src/utils/literal.ts` and `src/utils/string-generator.ts`. Transitive tests miss too many of their branches, and a wrong answer can change program behavior.
- `src/index.ts` exports the public API and nothing else.
...
- Put shared AST and path predicates in `src/utils/`. Put shared evaluation logic in `src/analysis/`. Import from those modules, never from one transform into another.
...
- Direct predicate tests in `tests/analysis/` and `tests/utils/` should cover a range of inputs, with extra weight on cases the helper must refuse. An over-eager `true` can break observational equivalence.
- Fix the code, not the test. If an assertion is wrong, replace it with a stronger one and explain why in the commit message.
- Every entry point in `src/transforms/` and `src/preparation/` needs an `@example` that demonstrates real behavior. Match the fixtures: `// ◀️ before`, the input, a blank line, `// ▶️ after`, then the output.
- `defineCases` in `tests/helpers.ts` writes `tests/fixtures/output/<name>.js` during a test run. Use those files to inspect each transform's isolated output.
